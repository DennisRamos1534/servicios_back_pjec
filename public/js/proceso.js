...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const urlPersonas = `${pathUrl}/api/reporte/procesoAll`;
            ...
            try {
                ...
                if(resultadoReporte['ok']) {
                    ...
                    resultadoReporte['reportes'].forEach(per => {
                        ...
                        tdBody1.innerHTML = `<a class="btnSeleccionar button ${(per['estado'] === 2) ? 'proceso seleccionar' : 'terminado'} proceso" href="#" data-id="${per['uid']}">${(per['estado'] === 2) ? 'terminar' : 'terminado'}</a>`;
    ...
    function reporteCompletar(e) {
        ...
        if(e.target.classList.contains('btnSeleccionar')) {
            ...
            const urlActualizar = `${pathUrl}/api/reporte/terminado/${id}`;

            Swal.fire({
                title: 'Estas seguro(a)?',
                // text: "Deseas acompletar el reporte?",
                text: "Deseas completar el reporte?",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    Swal.fire(
                    'Actualizado',
                    'El reporte se completo correctamente',
                    'success'
                    )
                }
            })
    ...
    async function hacerPeticion(e, urlActualizarPen) {
        try {
            ...
            if(resultadoActualizar["ok"] == true) {
                ...
                etiqueta.classList.remove("proceso");
    ...
    function eliminarReporte(e) {
        ...
        resultado.forEach(element => {
            ...
            row.innerHTML += `
                ...
                <td><a class=" button ${(element['estado'] === 2) ? 'proceso seleccionar' : 'terminado'} proceso" href="#" data-id="${element['uid']}">${(element['estado'] === 2) ? 'terminar' : 'terminado'}</a></td> 
                <td>${element['fecha']}</td>
